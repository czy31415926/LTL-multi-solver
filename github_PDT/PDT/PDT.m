function [best_stateP,best_stateB,best_stateAP,section,tree] = PDT(B,T,AG)
tree=[struct('BS',[1],'PS',[0],'ori',[],'traverse',0,'p',[],'t',[],'cost',0,'flag',0)];
for i=1:length(AG)
    tree(1).p=[tree(1).p;AG(i).p];
    tree(1).t=[tree(1).t;AG(i).t];
end
